...
author: "Wulan"
date: "Zeus Night Online: 24 & 25 September 2024"
output:
  html_document:
...
Apakah nilai mean di atas dapat diandalkan? Tidak, karena mengandung outlier
...
> Nilai median memiliki jarak yang cukup jauh dengan nilai mean. Sehingga apabila terdapat outlier, nilai median lebih representatif untuk digunakan
...
Pemahaman: Variance terbesar dimiliki oleh Candy A atau B? Candy B, Karena memiliki variasi warna yang lebih banyak
...
- Daerah A : Kemungkinan besar daerah perkantorannya lebih stabil
- Daerah B : Memiliki potensi untuk mendapatkan harga yang murah
...
> Harga bangunan senilai 800jt dianggap tidak wajar, mengingat daerah B memiliki nilai banguan yang wajar antara 239jt - 578jt
...
4. Bentuk distribusi data:
  + box yang berada di tengah = **distribusi normal**
...
> Data memiliki outlier, yaitu outlier atas
...
```{r}
cov(cc$Rating, cc$Income)
```

Interpretasi nilai: Memiliki covariance yang positif. Maka, Semakin tinggi income, rating credit juga akan semakin tinggi.
...
Jawaban: Memiliki korelasi yang positif kuat. Sehingga apabila income naik, maka rating juga akan naik
...
- **Central Tendency** = Mean, median, modus
  + Untuk data numerik: Mean dan Median
    - Ketika terdapat outlier:Median
    - Ketika tidak terdapat outlier:Mean
  + Untuk data kategorik : Modus
  
- **Measure of Spread** = Variance dan Standard Deviasi
  + Variance tidak dapat diinterpretasi secara sendirinya karena merupakan nilai kuadrat
  + Standard deviation memiliki satuan data asli, sehingga dapat di interpretasikan
  + Range dan IQR dapat divisualisasikan dengan boxplot
  
- **Variable Relationship**: Covariance dan Correlation
  ...
  + Correlation digunakan untuk mengukur kekuatan hubungan
    - Rangenya -1 s.d 1
    - semakin mendekati -1 -> strong negative correlation
    - semakin mendekati 1 -> strong positive correlation
...
- **Standar normal baku** adalah distribusi normal dengan mean = 0 dan standar deviasi = 1. 
...
Insight: data kita menyimpang 1.5 dari rata-ratanya
...
Insight: Peluang pria dewasa di Indonesia memiliki tinggi badan > 180 cm 0.06 (6.6%)
...
- alpa (tingkat error) : 100% - 95% = 5%, artinya kita mentoleransi error sebesar 5%, bahwa mungkin saja rata-rata Balance nasabah aslinya terletak di luar Confidence Interval
...
Kesimpulan: 
- Mean sample = 520
- Rata-rata Balance untuk populasi nasabah ada di rentang 474.43 - 565.56

Dengan tingkat keyakinan 95%, maka rata-rata Balance untuk nasabah Bank ABC ada di rentang 474.43 - 565.56
...
- $H_0$: Influencer tidak meningkatkan rata-rata jumlah likes
- $H_1$: Influencer meningkatkan rata-rata jumlah likes
...
> Dikarenakan p-value < alpha maka terima H1

**IV. Kesimpulan**

> Influencer mempengaruhi kenaikan rata-rata jumlah likes
